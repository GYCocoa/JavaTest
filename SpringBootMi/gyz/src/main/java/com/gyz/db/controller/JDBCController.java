...
import com.gyz.db.mapper.BannerMapper;
...
//    @PostMapping("/home/postList")
    ...
    //用post方式调用
    @PostMapping("home/queryBannerList")
    public MsgResponse findAll() {
        List<Banner> list;
        try {
            list = bannerMapper.queryBannerList();
    ...
    //用post方式调用
    @PostMapping("home/shopDetail")
    public MsgResponse shopDetail(@RequestParam("id") int id) {
        Map map;
        try {
            map = bannerMapper.queryShopDetail(id);
            if (map == null){
                return MsgResponse.error("请求错误！");
            }
